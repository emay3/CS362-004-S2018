...
int main(){
	...
	for(iteration = 0; iteration < ITERATIONS_LIMIT; iteration++){
		...
		game.hand[player][position] = smithy; 
		...
		result = cardEffect(smithy, choice1, choice2, choice3, &testState, position, 0);
		
		//test 1: cardEffect returned without error
		assertState("cardEffect Smithy returned", 0, result);
		
		//test 2: drew 3 cards to hand (technically this means that there the count should increase or decrease by 2 because of the discard of smithy)
		
		print)
		assertState("current player gained 3 cards to handCount[]", expState.handCount[player]+2, testState.handCount[player]);
		
		//test 3: drew those 3 cards from hand
		assertState("current player drew 3 cards from deck deckCount[]", expState.deckCount[player]-3, testState.deckCount[player]);
		
		//test4: discarded the smithy card 
		assertState("current player discarded smithy card", expState.discardCount[player}+1, testState.discardCount[player]);
		...
		assertState("current player deck+hand+discard count still same", cardsE, cardsT);
	...
	printf("\n********* Smithy RandomTest Summary: ***********");
	printf("\n Smithy card completed %d tests.", iteration);
